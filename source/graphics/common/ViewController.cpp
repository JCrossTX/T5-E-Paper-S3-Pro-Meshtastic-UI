 ...
 *
 */
bool ViewController::sendAdminMessage(meshtastic_AdminMessage &config, uint32_t nodeId, uint32_t requestId)
{
    meshtastic_Data_payload_t payload;
    payload.size = pb_encode_to_bytes(payload.bytes, DATA_PAYLOAD_LEN, &meshtastic_AdminMessage_msg, &config);
    // The id-carrying generic send: the packet goes out with id=requestId
    // and want_ack, and the local radio acks it back with that id in
    // decoded.request_id - which is what handleResponse keys on.
    return send(nodeId, 0, 0, requestId, meshtastic_PortNum_ADMIN_APP, true, false, payload.bytes, payload.size);
}
        ...
        if (setupDone || (from.which_payload_variant == meshtastic_FromRadio_config_tag &&
            ...
            case meshtastic_FromRadio_moduleConfig_tag: {
                ...
                case meshtastic_ModuleConfig_statusmessage_tag:
                    view->updateStatusMessageModule(module.payload_variant.statusmessage);
                    break;
                case meshtastic_ModuleConfig_traffic_management_tag:
                    view->updateTrafficManagementModule(module.payload_variant.traffic_management);
                    break;
                case meshtastic_ModuleConfig_tak_tag:
                    view->updateTakModule(module.payload_variant.tak);
                    break;
                case meshtastic_ModuleConfig_mesh_beacon_tag:
                    view->updateMeshBeaconModule(module.payload_variant.mesh_beacon);
                    break;
                default:
                    // Skip the variant, keep the packet: a firmware that adds a
                    // module config must not take the UI down with it.
                    ILOG_WARN("unsupported module config variant: %u - skipped",
            ...
            case meshtastic_FromRadio_region_presets_tag: {
                // patches/0027: the region/preset legality map this firmware
                // adds to the handshake (PhoneAPI.cpp:667-677). The 540x960
                // settings surface generates its picker tables from the protos
                // at build time, so the runtime copy is redundant here -
                // consumed so every handshake stops logging an error for a
                // message the firmware always sends.
                break;
            }
            default: {
