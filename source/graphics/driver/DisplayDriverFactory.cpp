...
#if defined(LGFX_DRIVER) || defined(LGFXConfig) || defined(ARCH_PORTDUINO)
...
#endif
// EINK_DRIVER only - not ARCH_PORTDUINO: the e-paper driver targets the
// ESP32 parallel bus and does not build for a host.
#if defined(EINK_DRIVER)
...
#endif
// Board adapter for the parallel e-paper path, alongside the LGFX configs.
#if defined(EINK_DRIVER) && defined(T5_S3_EPAPER_PRO)
#include "graphics/driver/T5EpaperBoard.h"
#endif
#ifdef T_HMI
...
#elif defined(EINK_DRIVER)
    // Embedded e-paper: tftSetup() reaches the factory here, with no config.
    // Geometry is the LOGICAL (portrait) size.
    return new EINKDriver<EINK_DRIVER>(width, height);
...
DisplayDriver *DisplayDriverFactory::create(const DisplayDriverConfig &cfg)
{
...
#endif
#if defined(EINKConfig) && defined(EINK_DRIVER)
    ...
    switch (cfg._device) {
#ifndef ARCH_PORTDUINO
#if !defined(LGFX_DRIVER) && !defined(EINK_DRIVER)
#error "LGFX_DRIVER or EINK_DRIVER must be defined!"
#endif
#if defined(T_HMI)
