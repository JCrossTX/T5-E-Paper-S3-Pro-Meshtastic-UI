...
static void epd_board_init(uint32_t epd_row_width, const EpdInitConfig* init_config) {
    ...
    // patches/0025: factory directions (lib/epdiy epd_board_v7.c:146,151).
    // PC12 is the side button on this board (io_extend.c "Button") and must
    // stay an input; port 0 is all outputs, exactly as factory sets it (the
    // output register keeps its 0xFF power-on default - factory writes no
    // port-0 values either).
    ESP_ERROR_CHECK(pca9555_set_config(
        config_reg.i2c.pca, CFG_PIN_PWRGOOD | CFG_PIN_INT | (PCA_PIN_PC12 >> 8), 1
    ));
    pca9555_set_config(config_reg.i2c.pca, 0x00, 0);
