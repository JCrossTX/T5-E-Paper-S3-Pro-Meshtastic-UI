...
void epd_renderer_init(enum EpdInitOptions options, const EpdInitConfig* config) {
    ...
    // patches/0026: zero the map. It is filled line-by-line by the feed
    // tasks while the LCD DMA consumer reads it from an ISR; if the
    // consumer outruns the feeders it reads unwritten entries, and a
    // garbage value asserts in retrieve_line_isr (render_lcd.c:38) and
    // reboots the chip - the first on-device boot of this port died in
    // that loop. Zeroed, a starved read lands on queue 0 and degrades to
    // epdiy's own EPD_DRAW_EMPTY_LINE_QUEUE handling: a logged error and
    // a blank line, not a crash.
    memset(render_context.line_threads, 0, rounded_display_height());
