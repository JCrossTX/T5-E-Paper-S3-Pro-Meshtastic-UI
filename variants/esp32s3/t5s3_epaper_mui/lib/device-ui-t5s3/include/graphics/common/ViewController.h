...
class ViewController
{
  public:
    ...
    // With a caller-chosen packet id, so the routing ACK comes back
    // correlatable (handleResponse receives it as decoded.request_id).
    virtual bool sendAdminMessage(meshtastic_AdminMessage &config, uint32_t nodeId, uint32_t requestId);
    virtual bool sendAdminMessage(meshtastic_AdminMessage &&config, uint32_t nodeId);
